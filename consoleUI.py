...
class ConsoleUI:
    ...
    def send_firmware_update(self, filename):
        """Handles firmware update command."""
        try:
            with open(filename, 'r') as file:
                data = file.read()
                self.serial_comm.send_serial_input(f"UPDATE {filename}\n")
                self.serial_comm.send_serial_input(data)
                self.log(f"[SYSTEM] Firmware update started for {filename}.")
        except Exception as e:
            self.log(f"[ERROR] Failed to read file: {e}")

    def browse_file(self):
        """Opens a file dialog to select a HEX file for update."""
        filename = filedialog.askopenfilename(filetypes=[("HEX Files", "*.hex")])
        if filename:
            self.input_entry.delete(0, tk.END)
            self.input_entry.insert(0, f"update {filename}")
